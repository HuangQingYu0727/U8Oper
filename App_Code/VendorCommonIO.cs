...
public class VendorCommonIO : System.Web.Services.WebService
{
    


    public VendorCommonIO()
    {
        //如果使用设计的组件，请取消注释以下行 
        //InitializeComponent(); 
    ...
    public string Pu_Vendor_GZ(string idlist,string cvencode,string cacc_id, string cmaker)
    {
        ...
        try
        {
            ...
            if (U8Operation.GetDataInt("select count(*) from VenDB..cc_fire_pugz where tempKey='" + idlist + "'", Cmd) > 200)
            {
                throw new Exception("一次性开票不允许超过200行。");
            }
            //获得制单人
            ...
            DataTable dtVouch = U8Operation.GetSqlDataTable(@"select a.cvencode,a.iexchrate,a.cexch_name,substring(a.rdtype,4,20) rdtype,min(autoid) autoid,a.cbustype,
                    	case when cc_gz_qty>=0 then 'blue' else 'red' end cColor,max(a.cptcode) cptcode
                ...
                where m.tempKey='" + idlist + "' and a.cvencode='" + cvencode + @"' and cc_HasCost='有价' and cc_kp_autoid is null
            ...
            for (int i = 0; i < dtVouch.Rows.Count; i++)
            {
                ...
                if (dtVouch.Rows[i]["cColor"] + "" == "blue")   //蓝色发票 处理折扣折让
                {
                    ...
                    from " + cDBOwer + @"..v_cqcc_gzd_mx_list a inner join " + cDBOwer + @"..T_CC_GZD_Pre b on a.rdtype=b.cc_rdtype and a.autoid=b.cc_autoid and a.cvencode=b.cc_vencode
                        inner join VenDB..cc_fire_pugz m on b.cc_identity=m.id
                    where m.tempKey='" + idlist + "' and a.cvencode='" + cvencode + "' and substring(a.rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                        and a.cexch_name='" + dtVouch.Rows[i]["cexch_name"] + "' and a.cbustype in('" + dtVouch.Rows[i]["cbustype"] + @"') 
                        and case when b.cc_gz_qty>=0 then 'blue' else 'red' end='blue'", Cmd);
                    if (float.Parse(cAllMoney) <= 0) 
                        throw new Exception("【" + cvencode + "】业务类型【" + dtVouch.Rows[i]["cbustype"] + "】折扣金额过大（或负金额），超出蓝字入库单金额");  //金额不够，不能开票
                }
                #region  //写发票主表
                ...
                if (dtVouch.Rows[i]["cColor"] + "" == "blue")
                {
                    dtBody = U8Operation.GetSqlDataTable(@"select b.cc_identity,autoid,left(rdtype,2) cpu_type,ivalidqty pu_qty,a.iquantity iqty,ioritaxcost,iorisum,b.cc_rate itaxrate,cfp_invcode cinvcode,
                        ...
                        where m.tempKey='" + idlist + "'  and a.cvencode='" + cvencode + "' and substring(a.rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                            ...
                            and case when b.cc_gz_qty>=0 then 'blue' else 'red' end='blue' order by autoid", "dtBody", Cmd);
                }
                else
                {   //红色发票
                    dtBody = U8Operation.GetSqlDataTable(@"select b.cc_identity,autoid,left(rdtype,2) cpu_type,ivalidqty pu_qty,a.iquantity iqty,ioritaxcost,iorisum,b.cc_rate itaxrate,cfp_invcode cinvcode,
