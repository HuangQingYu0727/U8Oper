...
public class VendorIO : System.Web.Services.WebService
{
    ...
    public static string SendResult(string code, string msg)
    {
        ...
        return DataTableToJson(dtResult);
    }
    ...
    public string JustPriceAdd(string jsonvalue,string cacc_id,string cmaker)
    {
        ...
        try
        {
            ...
            Cmd.ExecuteNonQuery();
            string cMaxCode = U8Operation.GetDataString("select 'JT" + cacc_id + "'+right('000000000'+cast(isnull(cast(replace(max(ccode),'JT" + cacc_id + "','') as int),0)+1 as varchar(20)),10) from " + dbname + "..PU_PriceJustMain where ccode like 'JT" + cacc_id + "%'", Cmd);
            ...
            for (int i = 0; i < dtJust.Rows.Count; i++)
            {
                ...
                DataTable dtPriceOldRow = U8Operation.GetSqlDataTable("select top 1 a.autoid,a.itaxunitprice,b.cdefine26,b.cdefine27 from " + dbname + @"..Ven_Inv_Price a 
                        left join " + dbname + @"..PU_PriceJustDetail b on a.ipriceautoid=b.autoid
                    where a.cvencode='" + dtJust.Rows[i]["cvencode"] + "' and a.cinvcode='" + dtJust.Rows[i]["cinvcode"] + @"' and a.dDisabledate is null 
                        order by a.dEnableDate desc", "dtPriceOldRow", Cmd);
...
//                         0,0,null,0" + dtJust.Rows[i]["itaxrate"] + ",0" + dtJust.Rows[i]["itaxcost"] + ",'人民币')";
                    Cmd.CommandText = "insert into " + dbname + @"..PU_PriceJustDetail(autoid,id,operationtype,cvencode,cinvcode,dstartdate,bsales,fminquantity,iunitprice,itaxrate,itaxunitprice,cexch_name,cbodymemo,bEndPriceList) 
                        values(" + cChildID + "," + cMID + ",0,'" + dtJust.Rows[i]["cvencode"] + "','" + dtJust.Rows[i]["cinvcode"] + @"',convert(varchar(7),getdate(),120)+'-01',
                         0,0,0" + dtJust.Rows[i]["itaxcost"] + ",0" + dtJust.Rows[i]["itaxrate"] + ",null,'人民币',convert(varchar(7),getdate(),120)+'新增；" + dtJust.Rows[i]["cdefine1"] + "',0)";
                }
                else
                {
                    //修改价格表
...
//                         0,0,null,0" + dtJust.Rows[i]["itaxrate"] + ",0" + dtJust.Rows[i]["itaxcost"] + ",'人民币'," + dtPriceOldRow.Rows[0]["autoid"] + ",0" + dtPriceOldRow.Rows[0]["itaxunitprice"] + ")";
                    Cmd.CommandText = "insert into " + dbname + @"..PU_PriceJustDetail(autoid,id,operationtype,cvencode,cinvcode,dstartdate,bsales,fminquantity,iunitprice,itaxrate,itaxunitprice,cexch_name,ipriceid,ijusttaxprice,cbodymemo,cdefine26,cdefine27,bEndPriceList) 
                        values(" + cChildID + "," + cMID + ",1,'" + dtJust.Rows[i]["cvencode"] + "','" + dtJust.Rows[i]["cinvcode"] + @"',convert(varchar(7),getdate(),120)+'-01',
                         0,0,0" + dtJust.Rows[i]["itaxcost"] + ",0" + dtJust.Rows[i]["itaxrate"] + ",null,'人民币'," + dtPriceOldRow.Rows[0]["autoid"] + ",0" + dtPriceOldRow.Rows[0]["itaxunitprice"] + @",
                         convert(varchar(7),getdate(),120)+'修改；" + dtJust.Rows[i]["cdefine1"] + @"',
                        " + (dtPriceOldRow.Rows[0]["cdefine26"] + "" == "" ? "NULL" : dtPriceOldRow.Rows[0]["cdefine26"] + "") + @",
                        " + (dtPriceOldRow.Rows[0]["cdefine27"] + "" == "" ? "NULL" : dtPriceOldRow.Rows[0]["cdefine27"] + "") + ",0)";
                }
                Cmd.ExecuteNonQuery();
                ...
                //Cmd.ExecuteNonQuery();
                Cmd.CommandText = "update " + dbname + @"..PU_PriceJustDetail set itaxunitprice=round(iunitprice*(100+itaxrate)/100.0,5) where autoid=" + cChildID;
                Cmd.ExecuteNonQuery();
            ...
            //补充无调价记录数据
            Cmd.CommandText = @"insert into " + dbname + @"..PU_PriceJustDetail(autoid,id,operationtype,ipriceid,cvencode,cinvcode,dstartdate,bsales,fminquantity,iunitprice,itaxrate,itaxunitprice,cexch_name,ijusttaxprice,cbodymemo,cdefine26,cdefine27,bEndPriceList)
                select " + cmax_child_id + "+(ROW_NUMBER() OVER(ORDER BY a.cinvcode)) autoid," + cMID + @" id,1,a.autoid ipriceid,a.cvencode,a.cinvcode,convert(varchar(7),getdate(),120)+'-01' dstartdate,0 bsales,0 fminquantity,a.iunitprice,
                    a.itaxrate,a.itaxunitprice,a.cexch_name,a.itaxunitprice,b.cbodymemo,b.cdefine26,b.cdefine27,0 bEndPriceList 
                from " + dbname + @"..Ven_Inv_Price a left join " + dbname + @"..PU_PriceJustDetail b on a.ipriceautoid=b.autoid 
                where a.cVenCode='" + c_vencode + @"' and a.dDisableDate is null and a.cInvCode not in(select cinvcode from " + dbname + @"..PU_PriceJustDetail where id=" + cMID + ")";
    ...
    public string Pu_Vendor_GZ(string yearmonth,string cvencode,string cacc_id, string cmaker)
    {
        ...
        try
        {
            ...
            //写临时表 含业务类型  存货编码汇总
            Cmd.CommandText = @"select a.cvencode,a.iexchrate,a.cexch_name,substring(a.rdtype,4,20) rdtype,min(autoid) autoid,a.cbustype,a.cinvcode,case when SUM(cc_gz_qty)>=0 then 'blue' else 'red' end cColor
                into #tmp_month_inv_sum
                ...
                where b.cc_yearmonth<='" + yearmonth + "' and a.cvencode='" + cvencode + @"' and a.cbustype not in('索赔','价格折让') and cc_HasCost='有价' and cc_kp_autoid is null
                    and b.cc_cinvcode<>'03024800001'
                group by a.cvencode,a.iexchrate,a.cexch_name,substring(a.rdtype,4,20),a.cbustype,a.cinvcode";
            Cmd.ExecuteNonQuery();
            
            //写发票
            DataTable dtVouch = U8Operation.GetSqlDataTable(@"select cvencode,iexchrate,cexch_name,rdtype,min(autoid) autoid,cbustype,cColor 
                from #tmp_month_inv_sum
                group by cvencode,iexchrate,cexch_name,rdtype,cbustype,cColor", "dtVouch", Cmd);
            ...
            for (int i = 0; i < dtVouch.Rows.Count; i++)
            {
                ...
                if (dtVouch.Rows[i]["cColor"] + "" == "blue")   //蓝色发票 处理折扣折让
                {
                    ...
                    where b.cc_yearmonth<='" + yearmonth + "' and a.cvencode='" + cvencode + "' and substring(a.rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                        ...
                        and a.cinvcode in(select cinvcode from #tmp_month_inv_sum where cexch_name='" + dtVouch.Rows[i]["cexch_name"] + "' and rdtype='" + dtVouch.Rows[i]["rdtype"] + @"' 
                            and cbustype='" + dtVouch.Rows[i]["cbustype"] + @"' and cColor='blue')", Cmd);
                ...
                if (dtVouch.Rows[i]["cColor"] + "" == "blue")
                {
                    dtBody = U8Operation.GetSqlDataTable(@"select b.cc_identity,autoid,left(rdtype,2) cpu_type,ivalidqty pu_qty,a.iquantity iqty,ioritaxcost,iorisum,b.cc_rate itaxrate,cfp_invcode cinvcode,
                        b.cc_taxmoney q_oriSum,ccode,kd_qty,kd_cost,b.cc_kd_money kd_money,b.cc_tax,cfree1,cfree2,b.cc_yearmonth
                        from " + cDBOwer + @"..v_cqcc_gzd_mx_list a inner join " + cDBOwer + @"..T_CC_GZD_Pre b on a.rdtype=b.cc_rdtype and a.autoid=b.cc_autoid and a.cvencode=b.cc_vencode
                        where b.cc_yearmonth<='" + yearmonth + "' and a.cvencode='" + cvencode + "' and substring(a.rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                            and cexch_name=" + pumain.cexch_name + @" and a.cbustype='" + dtVouch.Rows[i]["cbustype"] + @"' and b.cc_checker is not null
                            and a.cinvcode in(select cinvcode from #tmp_month_inv_sum where cexch_name='" + dtVouch.Rows[i]["cexch_name"] + "' and rdtype='" + dtVouch.Rows[i]["rdtype"] + @"' 
                                and cbustype='" + dtVouch.Rows[i]["cbustype"] + @"' and cColor='blue')
                        union all
                        select 0 cc_identity,0 autoid,left(rdtype,2) cpu_type,0 pu_qty,0 iqty,0,0 iorisum,a.itaxrate,cfp_invcode cinvcode,
	                        0 q_oriSum,'' ccode,0 kd_qty,0 kd_cost,sum(b.cc_kd_money) kd_money,sum(b.cc_tax) cc_tax,null cfree1,null cfree2,b.cc_yearmonth 
                        from " + cDBOwer + @"..v_cqcc_gzd_mx_list a inner join " + cDBOwer + @"..T_CC_GZD_Pre b on a.rdtype=b.cc_rdtype and a.autoid=b.cc_autoid and a.cvencode=b.cc_vencode
                        where b.cc_yearmonth<='" + yearmonth + "' and a.cvencode='" + cvencode + "' and substring(a.rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                            and b.cc_kd_money<>0 and b.cc_checker is not null
                        group by cfp_invcode,a.itaxrate,left(rdtype,2),b.cc_yearmonth", "dtBody", Cmd);
                }
                else
                {   //红色发票
                    dtBody = U8Operation.GetSqlDataTable(@"select b.cc_identity,autoid,left(rdtype,2) cpu_type,ivalidqty pu_qty,a.iquantity iqty,ioritaxcost,iorisum,b.cc_rate itaxrate,cfp_invcode cinvcode,
                        ...
                        where b.cc_yearmonth<='" + yearmonth + "' and a.cvencode='" + cvencode + "' and substring(a.rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                            and cexch_name=" + pumain.cexch_name + " and a.cbustype='" + dtVouch.Rows[i]["cbustype"] + @"' and b.cc_checker is not null
                            and a.cinvcode in(select cinvcode from #tmp_month_inv_sum where cexch_name='" + dtVouch.Rows[i]["cexch_name"] + "' and rdtype='" + dtVouch.Rows[i]["rdtype"] + @"' 
                                and cbustype='" + dtVouch.Rows[i]["cbustype"] + @"' and cColor='red')
                ...
                {
                    //判断是否 加工费扣点
                    if (dtBody.Rows[l]["cinvcode"] + "" == "03024800001" && dtBody.Rows[l]["cpu_type"] + "" == "sp")  
                    {
                        //加工费索赔
                        Cmd.CommandText = "update " + cDBOwer + @"..T_CC_GZD_Pre set cc_kp_autoid=-1,cc_fp_code=" + pumain.cPBVCode + @"
                            where cc_yearmonth<='" + yearmonth + "' and cc_vencode='" + cvencode + "' and SUBSTRING(cc_rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' 
                                and cc_cinvcode='" + dtBody.Rows[l]["cinvcode"] + @"' and cc_HasCost='有价' and cc_kp_autoid is null 
                                and cc_yearmonth='" + dtBody.Rows[l]["cc_yearmonth"] + "' and cc_kd_money<>0 ";
                        Cmd.ExecuteNonQuery();
                        continue;
                    }
                    ...
                    pudetail.cDefine25 = "'" + dtBody.Rows[l]["cc_yearmonth"] + "'";  //备挂月份
...
//                            //删除原作废记录
//                            Cmd.CommandText = "delete from " + cDBOwer + @"..hy_listreport where CHEADID=" + pudetail.PBVID + " and CBODYID=" + pudetail.ID + " and CVOUCHERTYPE='发票'";
//                            Cmd.ExecuteNonQuery();
...
//                            Cmd.ExecuteNonQuery();
                    ...
                    //更新备挂记录
                    if (float.Parse("" + dtBody.Rows[l]["cc_identity"]) == 0)  //扣款
                    {
                        Cmd.CommandText = "update " + cDBOwer + @"..T_CC_GZD_Pre set cc_kp_autoid=" + pudetail.ID + ",cc_fp_code=" + pumain.cPBVCode + @" 
                            where cc_yearmonth<='" + yearmonth + "' and cc_vencode='" + cvencode + "' and SUBSTRING(cc_rdtype,4,20)='" + dtVouch.Rows[i]["rdtype"] + @"' 
                                and cc_HasCost='有价' and cc_kp_autoid is null and cc_yearmonth='" + dtBody.Rows[l]["cc_yearmonth"] + "' and cc_kd_money<>0 ";
                        Cmd.ExecuteNonQuery();
                    }
                    else
                    {
